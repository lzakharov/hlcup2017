...
func main() {
	...
	r.HandleFunc("/users/{id:[0-9]+}", handlers.UpdateUser).Methods("POST")
	...
	r.HandleFunc("/locations/{id:[0-9]+}", handlers.UpdateLocation).Methods("POST")
	r.HandleFunc("/locations/new", handlers.CreateLocation).Methods("POST")

	r.HandleFunc("/visits/{id:[0-9]+}", handlers.GetVisit).Methods("GET")
	r.HandleFunc("/visits/{id:[0-9]+}", handlers.UpdateVisit).Methods("POST")
